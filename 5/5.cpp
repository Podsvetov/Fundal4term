...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   radioButton1 = CreateWindow(L"button", L"Button", // кнопка // радиокнопка -- можно выбрать из заранее заготовленного списка.
       WS_CHILD | WS_VISIBLE | BS_AUTORADIOBUTTON,
       10, 10, 120, 30, hWnd, (HMENU)ID_RADIO1, hInstance, NULL);
   radioButton2 = CreateWindow(L"button", L"Edit", // эдит как в первой лабе
       WS_CHILD | WS_VISIBLE | BS_AUTORADIOBUTTON,
       10, 40, 120, 30, hWnd, (HMENU)ID_RADIO2, hInstance, NULL);
   radioButton3 = CreateWindow(L"button", L"Label", //метка
       WS_CHILD | WS_VISIBLE | BS_AUTORADIOBUTTON,
       10, 70, 120, 30, hWnd, (HMENU)ID_RADIO3, hInstance, NULL);
   radioButton4 = CreateWindow(L"button", L"ComboBox", // отображает текстовое поле, которое позволяет пользователю выбрать элементы из списка или ввести новое значение.
       WS_CHILD | WS_VISIBLE | BS_AUTORADIOBUTTON,
       10, 100, 120, 30, hWnd, (HMENU)ID_RADIO4, hInstance, NULL);
   radioButton5 = CreateWindow(L"button", L"spinButton", //кнопка со стрелками
       WS_CHILD | WS_VISIBLE | BS_AUTORADIOBUTTON,
        ...
        if (addObject) {
            for (size_t i = 0; i < 5; i++) {
                if (BST_CHECKED == SendMessage(objects[i].getHWND(), BM_GETCHECK, 0, 0)) {
                    ...
                    switch (i){
                    case 0: 
                        str = L"button"; // Кнопка она и есть кнопка. Вкл-выкл.
                        break;

                    case 1 :
                        str = L"edit"; // Поле, в него можно вводить
                        break;

                    case 2:
                        str = L"STATIC"; // эта штука просто есть, ни инпута, ни аутпута.
                        break;
                        
                    case 3:
                        str = L"COMBOBOX"; // Чем-то похоже на эдит
                        break;

                    case 4:
                        str = L"SCROLLBAR"; // Ну, скроллбар, поле управления и стрелки прокрутки
                        break;
                    }
                    ...
                    if (isCorrect) {
                        ...
                        SetWindowText(objects[objects.size() - 1].getHWND(), L"QWERTY");
                        addObject = false;
                    }
                    break;
